...
albert_dir= './drive/My Drive/AI_COLAB/albert_base'
...
flags.DEFINE_string(
    "albert_config_file", os.path.join(albert_dir,'albert_config'),
    "The config json file corresponding to the pre-trained ALBERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", 'uland', "The name of the task to train.")

flags.DEFINE_string(
    "vocab_file", os.path.join(albert_dir,'30k-clean.vocab'),
    "The vocabulary file that the ALBERT model was trained on.")

flags.DEFINE_string("spm_model_file", os.path.join(albert_dir,'30k-clean.model'),
                    "The model file for sentence piece tokenization.")

flags.DEFINE_string(
    "output_dir", os.path.join(output,'albert'),
    "The output directory where the model checkpoints will be written.")

flags.DEFINE_string("cached_dir", os.path.join(output,'albert'),
...
def main(_):
  ...
  processors = {
      "uland": classifier_utils.UlandProcessor,
  }
  ...
  if FLAGS.do_train:
    ...
    if not tf.compat.v1.gfile.Exists(train_file):
  ...
  if FLAGS.do_eval:
    ...
    if not tf.compat.v1.gfile.Exists(eval_file):
      classifier_utils.file_based_convert_examples_to_features(
    ...
    def _best_trial_info():
      ...
      if tf.compat.v1.gfile.Exists(best_trial_info_file):
        with tf.compat.v1.gfile.GFile(best_trial_info_file, "r") as best_info:
    ...
    def _remove_checkpoint(checkpoint_path):
      for ext in ["meta", "data-00000-of-00001", "index"]:
        ...
        tf.compat.v1.gfile.Remove(src_ckpt)

    def _find_valid_cands(curr_step):
      filenames = tf.compat.v1.gfile.ListDirectory(FLAGS.output_dir)
      candidates = []
    ...
    writer = tf.compat.v1.gfile.GFile(output_eval_file, "w")
    while global_step < FLAGS.train_step:
      ...
      if not steps_and_files:
        ...
        for checkpoint in sorted(steps_and_files.items()):
          ...
          writer.write("=" * 50 + "\n")
          writer.flush()
          with tf.compat.v1.gfile.GFile(best_trial_info_file, "w") as best_info:
  ...
  if FLAGS.do_predict:
    ...
    with tf.compat.v1.gfile.GFile(output_predict_file, "w") as pred_writer,\
        tf.compat.v1.gfile.GFile(output_submit_file, "w") as sub_writer:
