...
# parser.add_argument('-i', '--train-v2.0.json', default=None,
#                     help='The input English SQuAD file', required=True)
# parser.add_argument('-o', '--translate.json', default=None,
...
def load_data():
    with open('train-v2.0.json', 'r', encoding='utf-8') as infile:
        squad_json = json.load(infile)
        squad_json = squad_json['data']

    # divide data into x parts
    div = floor(len(squad_json) / 4)
    ...
    divided_squad_json.append([squad_json[i] for i in range(div * (4 - 1), len(squad_json))])
    return divided_squad_json


def export_data(json_input):
    with open('translate,json', 'w', encoding='utf-8') as outfile:
