...
class UlandProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    print('PATH', os.path.join(data_dir, 'zalo_short.json'))
    X1 = []
    X2 = []
    Y = []
    with open(os.path.join(data_dir, 'train.json'), 'r', encoding='utf-8') as json_file:
...
def create_model(albert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    if task_name != "sts-b":
      ...
      logits = tf.compat.v1.squeeze(logits, [-1])
      predictions = logits
